...
proc demo_Subplots_ItemSharing*() =
  ...
  igCheckboxFlags("ImPlotSubplotFlags_ShareItems", flags.puint32, ImPlotSubplotFlags.ShareItems.uint32)
  igCheckboxFlags("ImPlotSubplotFlags_ColMajor", flags.puint32, ImPlotSubplotFlags.ColMajor.uint32)
  ...
  if ipBeginSubplots("##ItemSharing", rows, cols, ImVec2(x: -1,y: 400), flags):
    ...
    for  i in 0..<rows*cols:
      if ipBeginPlot("") :
        defer: ipEndPlot()
        var fc = 0.01f
        ipPlotLineG("common".cstring,SinewaveGetter,fc.addr,1000)
        for j in 0..<6:
          if id[j] == i:
            ...
            ipPlotLineG(label.ptz.cstringCast,SinewaveGetter,fj.addr,1000)
